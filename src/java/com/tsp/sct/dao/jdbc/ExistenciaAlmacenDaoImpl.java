...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT ID_ALMACEN, ID_CONCEPTO, EXISTENCIA, ESTATUS, EXISTENCIA_PESO FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( ID_ALMACEN, ID_CONCEPTO, EXISTENCIA, ESTATUS, EXISTENCIA_PESO ) VALUES ( ?, ?, ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET ID_ALMACEN = ?, ID_CONCEPTO = ?, EXISTENCIA = ?, ESTATUS = ?, EXISTENCIA_PESO = ? WHERE ID_ALMACEN = ? AND ID_CONCEPTO = ?";
	...
	/** 
	 * Index of column EXISTENCIA_PESO
	 */
	protected static final int COLUMN_EXISTENCIA_PESO = 5;
	...
	public ExistenciaAlmacenPk insert(ExistenciaAlmacen dto) throws ExistenciaAlmacenDaoException
	{
		...
		try {
			...
			if (dto.isExistenciaPesoModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
		
				sql.append( "EXISTENCIA_PESO" );
				values.append( "?" );
				modifiedCount++;
			}
			...
			if (dto.isExistenciaPesoModified()) {
				if (dto.isExistenciaPesoNull()) {
					stmt.setNull( index++, java.sql.Types.DOUBLE );
				} else {
					stmt.setDouble( index++, dto.getExistenciaPeso() );
				}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE EXISTENCIA_PESO = ? ORDER BY EXISTENCIA_PESO", new Object[] {  new Double(existenciaPeso) } );
	}
	...
	protected void populateDto(ExistenciaAlmacen dto, ResultSet rs) throws SQLException
	{
		...
		dto.setExistenciaPeso( rs.getDouble( COLUMN_EXISTENCIA_PESO ) );
		if (rs.wasNull()) {
			dto.setExistenciaPesoNull( true );
		}
	...
	protected void reset(ExistenciaAlmacen dto)
	{
		...
		dto.setExistenciaPesoModified( false );
	}
