...
public class PermisosDaoImpl extends AbstractDAO implements PermisosDao
{
	/** 
...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT ID_PERMISOS, NOMBRE, DESCRIPCION FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( ID_PERMISOS, NOMBRE, DESCRIPCION ) VALUES ( ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET ID_PERMISOS = ?, NOMBRE = ?, DESCRIPCION = ? WHERE ID_PERMISOS = ?";

	/** 
	 * SQL DELETE statement for this table
	 */
	protected final String SQL_DELETE = "DELETE FROM " + getTableName() + " WHERE ID_PERMISOS = ?";

	/** 
	 * Index of column ID_PERMISOS
	 */
	protected static final int COLUMN_ID_PERMISOS = 1;
	...
	/** 
	 * Index of primary-key column ID_PERMISOS
	 */
	protected static final int PK_COLUMN_ID_PERMISOS = 1;
	...
	public PermisosPk insert(Permisos dto) throws PermisosDaoException
	{
		...
		try {
			...
			_e.printStackTrace();
			throw new PermisosDaoException( "Exception: " + _e.getMessage(), _e );
		}
		finally {
	...
	public void update(PermisosPk pk, Permisos dto) throws PermisosDaoException
	{
		...
		try {
			...
			stmt.setString( index++, dto.getNombre() );
			stmt.setString( index++, dto.getDescripcion() );
			stmt.setInt( 4, pk.getIdPermisos() );
			int rows = stmt.executeUpdate();
			reset(dto);
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
	...
	public void delete(PermisosPk pk) throws PermisosDaoException
	{
		...
		try {
			...
			stmt.setInt( 1, pk.getIdPermisos() );
			int rows = stmt.executeUpdate();
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
		}
	...
	{
		return findByPrimaryKey( pk.getIdPermisos() );
	}
	...
	{
		Permisos ret[] = findByDynamicSelect( SQL_SELECT + " WHERE ID_PERMISOS = ?", new Object[] {  new Integer(idPermisos) } );
	...
	{
		return findByDynamicSelect( SQL_SELECT + " ORDER BY ID_PERMISOS", null );
	}
	 ...
	 * @param userConn
	 */
	public PermisosDaoImpl(final java.sql.Connection userConn)
	{
	 ...
	 */
	protected Permisos fetchSingleResult(ResultSet rs) throws SQLException
	{
		if (rs.next()) {
	 ...
	 */
	protected Permisos[] fetchMultiResults(ResultSet rs) throws SQLException
	{
		...
		Permisos ret[] = new Permisos[ resultList.size() ];
	 ...
	 */
	protected void populateDto(Permisos dto, ResultSet rs) throws SQLException
	{
	 ...
	 */
	protected void reset(Permisos dto)
	{
	}
