...
public class QuartzClienteDaoImpl extends AbstractDAO implements QuartzClienteDao
{
	/** 
...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT ID_QUARTZ, ID_CLIENTE_EVC, ID_CLIENTE_SISTEM_TERCERO, CLAVE, ID_SISTEMA_TERCERO, ID_EMPRESA FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( ID_QUARTZ, ID_CLIENTE_EVC, ID_CLIENTE_SISTEM_TERCERO, CLAVE, ID_SISTEMA_TERCERO, ID_EMPRESA ) VALUES ( ?, ?, ?, ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET ID_QUARTZ = ?, ID_CLIENTE_EVC = ?, ID_CLIENTE_SISTEM_TERCERO = ?, CLAVE = ?, ID_SISTEMA_TERCERO = ?, ID_EMPRESA = ? WHERE ID_QUARTZ = ?";
	...
	/** 
	 * Index of column ID_CLIENTE_EVC
	 */
	protected static final int COLUMN_ID_CLIENTE_EVC = 2;

	/** 
	 * Index of column ID_CLIENTE_SISTEM_TERCERO
	 */
	protected static final int COLUMN_ID_CLIENTE_SISTEM_TERCERO = 3;
	...
	public QuartzClientePk insert(QuartzCliente dto) throws QuartzClienteDaoException
	{
		...
		try {
			...
			if (dto.isIdClienteEvcModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
		
				sql.append( "ID_CLIENTE_EVC" );
				values.append( "?" );
				modifiedCount++;
			}
		
			if (dto.isIdClienteSistemTerceroModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
		
				sql.append( "ID_CLIENTE_SISTEM_TERCERO" );
				values.append( "?" );
				modifiedCount++;
			}
			...
			if (dto.isIdClienteEvcModified()) {
				if (dto.isIdClienteEvcNull()) {
					stmt.setNull( index++, java.sql.Types.INTEGER );
				} else {
					stmt.setInt( index++, dto.getIdClienteEvc() );
				}
		
			}
		
			if (dto.isIdClienteSistemTerceroModified()) {
				if (dto.isIdClienteSistemTerceroNull()) {
					stmt.setNull( index++, java.sql.Types.INTEGER );
				} else {
					stmt.setInt( index++, dto.getIdClienteSistemTercero() );
				}
			...
			_e.printStackTrace();
			throw new QuartzClienteDaoException( "Exception: " + _e.getMessage(), _e );
		}
		finally {
	...
	{
		QuartzCliente ret[] = findByDynamicSelect( SQL_SELECT + " WHERE ID_QUARTZ = ?", new Object[] {  new Integer(idQuartz) } );
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_CLIENTE_EVC = ? ORDER BY ID_CLIENTE_EVC", new Object[] {  new Integer(idClienteEvc) } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_CLIENTE_SISTEM_TERCERO = ? ORDER BY ID_CLIENTE_SISTEM_TERCERO", new Object[] {  new Integer(idClienteSistemTercero) } );
	}
	...
	/**
	 * Method 'QuartzClienteDaoImpl'
	 * 
	 */
	public QuartzClienteDaoImpl()
	{
	}
	 ...
	 * @param userConn
	 */
	public QuartzClienteDaoImpl(final java.sql.Connection userConn)
	{
	...
	{
		return "quartz_cliente";
	}

	/** 
	 * Fetches a single row from the result set
	 */
	protected QuartzCliente fetchSingleResult(ResultSet rs) throws SQLException
	{
		if (rs.next()) {
			QuartzCliente dto = new QuartzCliente();
			populateDto( dto, rs);
			return dto;
		} else {
			return null;
	 ...
	 */
	protected QuartzCliente[] fetchMultiResults(ResultSet rs) throws SQLException
	{
		...
		QuartzCliente ret[] = new QuartzCliente[ resultList.size() ];
	 ...
	 */
	protected void populateDto(QuartzCliente dto, ResultSet rs) throws SQLException
	{
		dto.setIdQuartz( rs.getInt( COLUMN_ID_QUARTZ ) );
		dto.setIdClienteEvc( rs.getInt( COLUMN_ID_CLIENTE_EVC ) );
		if (rs.wasNull()) {
			dto.setIdClienteEvcNull( true );
		}
		
		dto.setIdClienteSistemTercero( rs.getInt( COLUMN_ID_CLIENTE_SISTEM_TERCERO ) );
		if (rs.wasNull()) {
			dto.setIdClienteSistemTerceroNull( true );
		}
	 ...
	 */
	protected void reset(QuartzCliente dto)
	{
		dto.setIdQuartzModified( false );
		dto.setIdClienteEvcModified( false );
		dto.setIdClienteSistemTerceroModified( false );
