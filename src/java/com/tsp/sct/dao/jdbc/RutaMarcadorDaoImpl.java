...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT ID_RUTA_MARCADOR, ID_RUTA, INFORMACION_MARCADOR, LATITUD_MARCADOR, LONGITUD_MARCADOR, ID_PROSPECTO, ID_CLIENTE, IS_VISITADO, DISTANCIA_PUNTO FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( ID_RUTA_MARCADOR, ID_RUTA, INFORMACION_MARCADOR, LATITUD_MARCADOR, LONGITUD_MARCADOR, ID_PROSPECTO, ID_CLIENTE, IS_VISITADO, DISTANCIA_PUNTO ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET ID_RUTA_MARCADOR = ?, ID_RUTA = ?, INFORMACION_MARCADOR = ?, LATITUD_MARCADOR = ?, LONGITUD_MARCADOR = ?, ID_PROSPECTO = ?, ID_CLIENTE = ?, IS_VISITADO = ?, DISTANCIA_PUNTO = ? WHERE ID_RUTA_MARCADOR = ?";
	...
	/** 
	 * Index of column DISTANCIA_PUNTO
	 */
	protected static final int COLUMN_DISTANCIA_PUNTO = 9;
	...
	public RutaMarcadorPk insert(RutaMarcador dto) throws RutaMarcadorDaoException
	{
		...
		try {
			...
			if (dto.isDistanciaPuntoModified()) {
				if (modifiedCount>0) {
					sql.append( ", " );
					values.append( ", " );
				}
		
				sql.append( "DISTANCIA_PUNTO" );
				values.append( "?" );
				modifiedCount++;
			}
			...
			int index = 1;
			if (dto.isIdRutaMarcadorModified()) {
				stmt.setInt( index++, dto.getIdRutaMarcador() );
			}
			...
			if (dto.isInformacionMarcadorModified()) {
				stmt.setString( index++, dto.getInformacionMarcador() );
			}
			...
			if (dto.isDistanciaPuntoModified()) {
				if (dto.isDistanciaPuntoNull()) {
					stmt.setNull( index++, java.sql.Types.DOUBLE );
				} else {
					stmt.setDouble( index++, dto.getDistanciaPunto() );
				}
			...
			if (rs != null && rs.next()) {
				dto.setIdRutaMarcador( rs.getInt( 1 ) );
			}
	...
	public void update(RutaMarcadorPk pk, RutaMarcador dto) throws RutaMarcadorDaoException
	{
		...
		try {
			...
			stmt.setInt( index++, pk.getIdRutaMarcador() );
			int rows = stmt.executeUpdate();
			reset(dto);
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
	...
	public void delete(RutaMarcadorPk pk) throws RutaMarcadorDaoException
	{
		...
		try {
			...
			stmt.setInt( 1, pk.getIdRutaMarcador() );
			int rows = stmt.executeUpdate();
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
		}
	...
	{
		RutaMarcador ret[] = findByDynamicSelect( SQL_SELECT + " WHERE ID_RUTA_MARCADOR = ?", new Object[] {  new Integer(idRutaMarcador) } );
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_RUTA_MARCADOR = ? ORDER BY ID_RUTA_MARCADOR", new Object[] {  new Integer(idRutaMarcador) } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE DISTANCIA_PUNTO = ? ORDER BY DISTANCIA_PUNTO", new Object[] {  new Double(distanciaPunto) } );
	}
	...
	{
		dto.setIdRutaMarcador( rs.getInt( COLUMN_ID_RUTA_MARCADOR ) );
		dto.setIdRuta( rs.getInt( COLUMN_ID_RUTA ) );
		dto.setInformacionMarcador( rs.getString( COLUMN_INFORMACION_MARCADOR ) );
		...
		dto.setDistanciaPunto( rs.getDouble( COLUMN_DISTANCIA_PUNTO ) );
		if (rs.wasNull()) {
			dto.setDistanciaPuntoNull( true );
		}
	...
	protected void reset(RutaMarcador dto)
	{
		...
		dto.setDistanciaPuntoModified( false );
	}
