...
public class EstatusComprobanteDaoImpl extends AbstractDAO implements EstatusComprobanteDao
{
	/** 
...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT ID_ESTATUS_COMPROBANTE, NOMBRE, DESCRIPCION FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( ID_ESTATUS_COMPROBANTE, NOMBRE, DESCRIPCION ) VALUES ( ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET ID_ESTATUS_COMPROBANTE = ?, NOMBRE = ?, DESCRIPCION = ? WHERE ID_ESTATUS_COMPROBANTE = ?";

	/** 
	 * SQL DELETE statement for this table
	 */
	protected final String SQL_DELETE = "DELETE FROM " + getTableName() + " WHERE ID_ESTATUS_COMPROBANTE = ?";

	/** 
	 * Index of column ID_ESTATUS_COMPROBANTE
	 */
	protected static final int COLUMN_ID_ESTATUS_COMPROBANTE = 1;
	...
	/** 
	 * Index of primary-key column ID_ESTATUS_COMPROBANTE
	 */
	protected static final int PK_COLUMN_ID_ESTATUS_COMPROBANTE = 1;
	...
	public EstatusComprobantePk insert(EstatusComprobante dto) throws EstatusComprobanteDaoException
	{
		...
		try {
			...
			int index = 1;
			stmt.setInt( index++, dto.getIdEstatusComprobante() );
			...
			_e.printStackTrace();
			throw new EstatusComprobanteDaoException( "Exception: " + _e.getMessage(), _e );
		}
		finally {
	...
	public void update(EstatusComprobantePk pk, EstatusComprobante dto) throws EstatusComprobanteDaoException
	{
		...
		try {
			...
			stmt.setString( index++, dto.getNombre() );
			stmt.setString( index++, dto.getDescripcion() );
			stmt.setInt( 4, pk.getIdEstatusComprobante() );
			int rows = stmt.executeUpdate();
			reset(dto);
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
	...
	public void delete(EstatusComprobantePk pk) throws EstatusComprobanteDaoException
	{
		...
		try {
			...
			stmt.setInt( 1, pk.getIdEstatusComprobante() );
			int rows = stmt.executeUpdate();
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
		}
	...
	{
		return findByPrimaryKey( pk.getIdEstatusComprobante() );
	}
	...
	{
		EstatusComprobante ret[] = findByDynamicSelect( SQL_SELECT + " WHERE ID_ESTATUS_COMPROBANTE = ?", new Object[] {  new Integer(idEstatusComprobante) } );
	...
	{
		return findByDynamicSelect( SQL_SELECT + " ORDER BY ID_ESTATUS_COMPROBANTE", null );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE ID_ESTATUS_COMPROBANTE = ? ORDER BY ID_ESTATUS_COMPROBANTE", new Object[] {  new Integer(idEstatusComprobante) } );
	}
	...
	/**
	 * Method 'EstatusComprobanteDaoImpl'
	 * 
	 */
	public EstatusComprobanteDaoImpl()
	{
	}
	 ...
	 * @param userConn
	 */
	public EstatusComprobanteDaoImpl(final java.sql.Connection userConn)
	{
	...
	{
		return "ESTATUS_COMPROBANTE";
	}

	/** 
	 * Fetches a single row from the result set
	 */
	protected EstatusComprobante fetchSingleResult(ResultSet rs) throws SQLException
	{
		if (rs.next()) {
			EstatusComprobante dto = new EstatusComprobante();
			populateDto( dto, rs);
			return dto;
		} else {
			return null;
	 ...
	 */
	protected EstatusComprobante[] fetchMultiResults(ResultSet rs) throws SQLException
	{
		...
		EstatusComprobante ret[] = new EstatusComprobante[ resultList.size() ];
	 ...
	 */
	protected void populateDto(EstatusComprobante dto, ResultSet rs) throws SQLException
	{
		dto.setIdEstatusComprobante( rs.getInt( COLUMN_ID_ESTATUS_COMPROBANTE ) );
	 ...
	 */
	protected void reset(EstatusComprobante dto)
	{
	}
