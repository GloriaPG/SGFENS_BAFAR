<%-- 
    Document   : catEmbalajes_list
    Created on : 9/11/2012, 10:54:43 AM
    Author     : Leonardo
--%>
...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    int idEmbalaje = -1;
    try{ idEmbalaje = Integer.parseInt(request.getParameter("idEmbalaje")); }catch(NumberFormatException e){}
    ...
    */
    ...
    try{
     ...
     Embalaje[] embalajesDto = new Embalaje[0];
     try{
         limiteRegistros = embalajeBO.findEmbalajes(idEmbalaje, idEmpresa , 0, 0, filtroBusqueda).length;
        ...
        embalajesDto = embalajeBO.findEmbalajes(idEmbalaje, idEmpresa,
    ...
    * Datos de catálogo
    */
    String urlTo = "../catEmbalajes/catEmbalajes_form.jsp";
    String paramName = "idEmbalaje";
                    ...
                    <div class="onecolumn">
						...
						<tbody>
							<tr>
                                                            <td>
                                                                <div id="search">
                                                                <form action="catEmbalajes_list.jsp" id="search_form" name="search_form" method="get">
                        ...
                        <div class="content">
                            <form id="form_data" name="form_data" action="" method="post">
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            for (Embalaje item:embalajesDto){
                                                try{
                                        %>
                                        <tr <%=(item.getIdEstatus()!=1)?"class='inactive'":""%>>
                                            <!--<td><input type="checkbox"/></td>-->
                                            <td><%=item.getIdEmbalaje() %></td>
                                            <td><%=item.getNombre() %></td>
                                            <td><%=item.getDescripcion()%></td>                                            
                                            <td>
                                                <%if(RolAutorizacionBO.permisoAccionesElemento(user.getUser().getIdRoles())){%>
                                                <a href="<%=urlTo%>?<%=paramName%>=<%=item.getIdEmbalaje()%>&acc=1&pagina=<%=paginaActual%>"><img src="../../images/icon_edit.png" alt="editar" class="help" title="Editar"/></a>
                                                &nbsp;&nbsp;
