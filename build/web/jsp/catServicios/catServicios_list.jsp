...
if (user == null || !user.permissionToTopicByURL(request.getRequestURI().replace(request.getContextPath(), ""))) {
    ...
    if (!buscar.trim().equals(""))
        filtroBusqueda = " AND (NOMBRE LIKE '%" + buscar + "%' OR DESCRIPCION LIKE '%" +buscar+"%' OR PRECIO LIKE '%" +buscar+"%' OR SKU LIKE '%" +buscar+"%')";
    
    int idServicio = -1;
    try{ idServicio = Integer.parseInt(request.getParameter("idServicio")); }catch(NumberFormatException e){}
    ...
    */
    ...
    try{
     ...
     Servicio[] serviciosDto = new Servicio[0];
     try{
         limiteRegistros = servicioBO.findServicios(idServicio, idEmpresa , 0, 0, filtroBusqueda).length;
        ...
        serviciosDto = servicioBO.findServicios(idServicio, idEmpresa,
    ...
    * Datos de catálogo
    */
    String urlTo = "../catServicios/catServicios_form.jsp";
    String paramName = "idServicio";
							...
							<tr>
                                                            <td>
                                                                ...
                                                                <form action="catServicios_list.jsp" id="search_form" name="search_form" method="get">
                                                                        <input type="text" id="q" name="q" title="Buscar por Nombre / Descripción / PRECIO / SKU" class="" style="width: 300px; float: left; "
                                                                               value="<%=buscar%>"/>
                        ...
                        <div class="content">
                            <form id="form_data" name="form_data" action="" method="post">
                                <table class="data" width="100%" cellpadding="0" cellspacing="0">
                                    ...
                                    <tbody>
                                        <% 
                                            for (Servicio item:serviciosDto){
                                                try{
                                        %>
                                        <tr <%=(item.getIdEstatus()!=1)?"class='inactive'":""%>>
                                            <!--<td><input type="checkbox"/></td>-->
                                            <td><%=item.getIdServicio() %></td>
                                            <td><%=item.getSku()%></td>
                                            ...
                                            <td>
                                                ...
                                                <a href="<%=urlTo%>?<%=paramName%>=<%=item.getIdServicio()%>&acc=1"><img src="../../images/icon_edit.png" alt="editar" class="help" title="Editar"/></a>
                                                &nbsp;&nbsp;
                            ...
                            <jsp:include page="../include/reportExportOptions.jsp" flush="true">
                                <jsp:param name="idReport" value="<%= ReportBO.SERVICIO_REPORT %>" />
